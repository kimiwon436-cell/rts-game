...
- `.env` 파일이 없으면 서버와 클라이언트 모두 **개발 모드**로 동작합니다. Firebase 없이, 이 브라우저에만 저장되는 연습용 계정(아이디 + 비밀번호)으로 가입·로그인합니다.
...
2. **Authentication → 로그인 방법**에서 **이메일/비밀번호**를 사용 설정합니다.
   게임은 **아이디 + 비밀번호**로 가입·로그인하지만, 내부적으로 아이디를 `아이디@id.<프로젝트ID>.firebaseapp.com` 주소로 바꿔
   이 방식을 씁니다. 콘솔의 사용자 목록에도 이 주소로 보입니다. 메일은 오가지 않으므로 비밀번호 재설정은 없습니다.

import { encodeBuilding, encodePlayer, encodeUnit } from '@rune/shared/snapshot.js';

/** 이번 틱에 모든 플레이어가 함께 보는 부분. 틱마다 한 번만 만든다. */
export function buildSharedFrame(world, events) {
  return {
    t: world.tick,
    players: world.players.filter(Boolean).map((p) => [p.slot, p.age]),
...
/** 플레이어별 스냅샷: 공유 부분 + 내 자원 (상대 자원은 보내지 않는다) */
export function snapshotFor(frame, world, slot) {
  return { ...frame, me: encodePlayer(world.players[slot]) };
}

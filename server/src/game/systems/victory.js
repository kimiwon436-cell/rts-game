 ...
 * - 카운트다운이 끝나거나, 유닛도 건물도 하나 남지 않으면 패배
 * - 패배하지 않은 플레이어가 한 명만 남으면 그 플레이어가 이긴다
 */
export function updateVictory(world) {
  ...
  const standing = players.filter((p) => !p.defeated);
  if (players.length >= 2 && standing.length <= 1) {
    const loser = players.find((p) => p.defeated);
    world.result = { winner: standing[0]?.slot ?? null, reason: loser.defeatReason, tick: world.tick };
  }
}

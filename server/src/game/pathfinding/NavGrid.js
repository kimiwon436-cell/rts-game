 ...
 * 배는 따로 물길 격자를 쓴다 (blocks에 "물이 아닌 곳"을 준다. 건물은 물에 서지 않으니 늘 그대로다).
 */
export class NavGrid {
  constructor(width, height, tiles, blocks = isBlockingTerrain) {
    this.width = width;
    this.height = height;
    this.blocked = new Uint8Array(width * height);
    for (let i = 0; i < tiles.length; i++) this.blocked[i] = blocks(tiles[i]) ? 1 : 0;
    /**

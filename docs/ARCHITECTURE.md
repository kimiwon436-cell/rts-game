...
└─ client/
   ...
   └─ src/
      ...
      │  ├─ entities.js      # 유닛·건물·금광 그리기 입구, 전투 효과, 선택 고리·체력 막대
      │  ├─ art/             # 그림: paint(재질) · sprites(캐시·외곽선) · unitArt · buildingArt · flag
      │  └─ minimap.js
      ├─ input/
      │  ├─ Input.js         # 마우스·키보드 상태
...
## 5-10. 그림 (`client/src/render/art/`, 확인: `npm run dev` → `/gallery.html`)

- **코드로 그린 채색 미니어처**: 이미지 파일 없이 Canvas로 그린다. 빛은 왼쪽 위에서 (그라디언트 음영), 실루엣 둘레에 짙은 외곽선,
  팀 색은 옷·망토·방패·말 덮개·돛·깃발에 크게 넣어 멀리서도 편이 보이게 한다.
- **스프라이트 캐시** (`sprites.js`): 그림은 처음 쓸 때 한 번 2배 해상도로 그려 오프스크린 캔버스에 담고, 매 프레임에는 복사만 한다.
  열쇠 = 종류 · 팀 색 · 동작 · 프레임 (· 변형: 농노 짐·도구, 근위병 방패벽, 아르카논 뿌리). 왼쪽을 볼 때는 뒤집어 그린다.
  유닛 300기 한 프레임 약 1.3ms (이 PC). 새 그림은 프레임당 6장까지만 그리고, 넘치면 서 있는 그림으로 대신한다 (첫 대규모 전투에서 끊기지 않게)
- **유닛 동작** (`unitArt.js`의 `unitPose`): 서 있음 · 걷기 4프레임(실제로 움직일 때만 — `ClientWorld`의 `still`) · 싸울 자세 ·
  공격 3프레임(`ATTACK` 이벤트를 받은 순간부터 0.36초, 친 쪽을 바라본다 — `attackAt`) · 농노 일(도끼·곡괭이·망치). 공중은 날갯짓, 배는 노 젓기.
  빛나는 곳(마법 구슬·성창·룬)은 스프라이트가 자리를 알려 주고 매 프레임 더해 그린다 (`glow`)
- **건물** (`buildingArt.js`): 종류·팀 색·(영주관은 시대)별 스프라이트. 풋프린트 아래쪽이 앞벽, 지붕·탑은 위로 솟는다.
  깃발(펄럭임)·굴뚝 연기·마법 빛·성소 제단 불은 매 프레임. 짓는 중: 터(말뚝·줄) → 아래부터 차오르는 벽 + 비계.
  체력 60% 밑이면 검은 연기, 35% 밑이면 불. 영주관 이름표는 그림 꼭대기 위(`buildingLabelY`)
- 모든 유닛·건물 종류에 그림이 있는지 테스트가 본다 (`client/test/art.test.js` — 그림이 없으면 그리다 멈춘다)

---

## 5-11. 팀별 길 (`server/src/game/World.js`)

- 우리 팀 건물은 지나가고 상대 팀 건물은 막히므로, 뭍 격자를 팀마다 따로 둔다 (`teamNav[팀]`). 건물은 **주인 팀 격자만 빼고** 막는다.
  금광·막힌 지형·쓰러진 나무는 모든 격자에 같이 반영한다 (`setNav`).
- `nav`(모든 건물이 막는 격자)는 유닛이 **서 있을** 자리를 고를 때 쓴다: 무리 이동의 목적지 칸, 생산·탑승 해제 자리, 건물에서 밀어내기,
  밀어내기(separation)의 이동. 그래서 우리 건물을 지나가도 그 안에 멈춰 서지는 않는다.
- `navOf(unit)`: 뭍 유닛은 자기 팀 격자, 배는 물길, 공중은 하늘길. 격자마다 A*를 하나씩 둔다 (`pathfinders`).
...
- 공중 유닛(`UNITS[type].flying`)은 **아무것도 막지 않는 격자**(`world.airNav`)를 쓰고, `moveUnit`이 길찾기 없이
  목표로 가는 웨이포인트 하나를 바로 준다. 지형·건물·유닛을 무시하고 직선으로 난다 (A* 비용 0).
- 밀어내기는 다니는 곳(`domainOf`: 뭍·물·하늘)이 같은 유닛끼리만 한다. 공중 유닛은 건물 자리에서 밀려나지도 않는다.
- 뭍·물의 유닛이 공중을 쫓을 때는 목표 바로 아래에서 가장 가까운 제 쪽 칸까지만 간다 (배가 뭍의 적을 쫓을 때와 같은 규칙).
- 피해 규칙으로 균형을 잡는다: 근접 공격은 비행에 0, 관통은 ×2, 공성은 비행에 0. 그래서 하늘은 장궁병·감시탑·갤리가 잡고,
  비룡(공성)은 건물을 부수되 하늘 싸움에는 끼지 못한다.
- 화면에서는 `AIR_ALTITUDE`(22px)만큼 위에 그리고 땅에 그림자를 깐다. 선택 고리는 땅(그림자 자리), 체력 막대와 투사체는 뜬 높이에 맞춘다.

### 배의 길찾기 (해군)

- 배는 **물길 격자**(`world.waterNav`: 물·다리만 열림)와 따로 둔 `waterPathfinder`로 길을 찾는다. 건물은 물에 서지 않아 물길은 경기 내내 그대로다.
  `world.navOf(unit)`가 유닛마다 격자를 고르고, 이동·밀어내기·경로 재탐색·스폰·목적지 칸(`destinationSlots`)·가까운 빈 칸(`nearestFreeTile`)이 모두 그 격자를 쓴다.
- 무리 이동 명령은 배와 뭍 유닛을 나눠 각자 격자에서 자리를 잡는다. 뭍을 찍으면 배는 48칸 안의 가장 가까운 물에서 시작한다.
- 물과 뭍 사이의 추격(배가 뭍의 적을, 장궁병이 배를): 목표에서 8칸 안의 **내 쪽 가장 가까운 칸(물가)**으로만 간다.
  닿을 수 없으면 공격 명령을 푼다. 근접 공격은 함선 방어에 0이라 스스로도 노리지 않고 명령도 거부된다.
- 밀어내기는 같은 쪽(배끼리, 뭍 유닛끼리)만 한다.
- **태우기** (`shared/src/rules/garrison.js`): 태우는 쪽 데이터 `garrison { capacity, riders, rangeBonus, ridersFight, sinks }`.
  아르카논은 정해진 4종·사거리 +2·등 위에서 싸움·쓰러지면 내림, 수송선은 뭍 유닛 누구나(궁극 제외)·싸우지 않음·가라앉으면 함께 잃음.
  배에 타러 가는 유닛은 배에서 가장 가까운 뭍으로 가서 기다리고, 닿지 못하면 1초마다만 길을 다시 찾는다.
